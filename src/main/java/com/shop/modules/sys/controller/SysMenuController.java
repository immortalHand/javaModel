...
public class SysMenuController extends AbstractController {
	...
	public List<SysMenuDto> list(){
		List<SysMenuDto> menuList = sysMenuService.selectList(null);
		for(SysMenuDto sysMenuEntity : menuList){
			SysMenuDto parentMenuEntity = sysMenuService.selectById(sysMenuEntity.getParentId());
	...
	public R select(){
		//查询列表数据
		List<SysMenuDto> menuList = sysMenuService.queryNotButtonList();
	...
	public R info(@PathVariable("menuId") Long menuId){
		SysMenuDto menu = sysMenuService.selectById(menuId);
	...
	public R delete(@PathVariable("menuId") long menuId){
		...
		//判断是否有子菜单或按钮
		List<SysMenuDto> menuList = sysMenuService.queryListParentId(menuId);
		if(menuList.size() > 0){
	...
	private void verifyForm(SysMenuDto menu){
		...
		if(menu.getParentId() != 0){
			SysMenuDto parentMenu = sysMenuService.selectById(menu.getParentId());
